...
- [Git 基本教程](#Git-基本教程)
    - [Git 常用命令](#Git-常用命令)
    - [Git 提交规范](#Git-提交规范)
- [Workflow/Gitflow 工作流](#Workflow/Gitflow-工作流)
- [代码书写规范](#代码书写规范)
- [开发实践](#开发实践)
    - [配置开发环境](#配置开发环境)
    - [获取项目代码](#获取项目代码)
...
我们推荐本地开发环境使用 VirtualBox + Vagrant 集成的 Centos7，并且统一使用 Docker 部署本地项目。

关于本地环境的搭建，我们已经有一份详细的安装教程 [docker_vagrant_centos7](http://git.epweike.net:3000/epwk/docker_vagrant_centos7)。
...
- 为什么使用 Docker
	> 避免出现「Work on my machine」。在开发过程中，可能由于环境的不一致导致在你的电脑上运行的程序没有问题，但是到线上就出现了各种各样的毛病，为了避免这种问题，应该使用统一的 Docker 容器

## Docker 基本教程

- 这里有一份很详细的 [Docker — 从入门到实践](https://yeasy.gitbooks.io/docker_practice/)
- 我们本地的开发环境，使用 [docker-compose](https://docs.docker.com/compose/overview/) 编排容器
...
- 提交规范参见 [how-to-commit](../git/how-to-commit.md)

## Workflow/Gitflow 工作流

- 什么是 [Gitflow工作流](https://nvie.com/posts/a-successful-git-branching-model/)
- 适合我们团队的 [Workflow](../README.md)
...
我们以[「优帆远扬」](https://learnku.com/docs/laravel-specification/5.5)团队的代码规范为基础，整理出适合我们团队的 [代码规范](http://git.epweike.net:3000/epwk/laravel_code_specification)。
...
Vagrant 启动成功时会生成一个 `webroot` 目录，我们规定项目必须检出到 `webroot` 目录下。
...
在 `/data/nginx/vhost` 目录底下，我们将提供尽可能全的项目的 Nginx 配置文件，并且以 `.example` 结尾，如果你需要某个项目的配置文件，拷贝一份并且重命名去掉 `.example` 即可。

如果代码需要 `composer` 安装，则可以使用：
...
> 关于 host 的配置工具，可以使用 [Hozz](https://blog.zhangruipeng.me/Hozz/)。

### 功能开发

`feature` 是功能分支，包括新功能的开发、周版本都应该创建相应的 `feature` 分支。

如果你已经了解 `Workflow`，你应当知道，要做一个新功能，需要从 `origin/develop` 创建一个 `feature/xxx` 分支：
...
在你完成你的功能分支之后，你应该把你提交的代码 `push` 到对应远程分支：

```
git push origin feature/new_doc
```

> 代码的提交规范一定要遵循 [代码书写规范](#代码书写规范)，`commit` 一定要遵循 [Git 提交规范](#Git-提交规范)，否则代码会推送不上远程仓库，遇见这种请款个，请参考 [代码不规范或者提交不规范如何重新提交](#代码不规范或者提交不规范如何重新提交)。

此时你在功能分支的测试环境测试没有问题之后，应在 `gogs` 对应项目的 `origin/develop` 发起合并请求，以范例项目（http://git.epweike.net:3000/suyaqi/teach.epweike.net/pulls）为例：
...
在发起代码合并之后，小组组长需要进行相应的代码审查以及合并请求操作：

![merge](../images/quick-start-merge.png)

如果项目要上线，运维人员需要从 `origin/develop` 向 `origin/master` 发起 `pull request`，这里姑且不体现相关操作。

### BUG 修复

BUG 修复是开发的日常，如果线上有功能 BUG 需要及时修复，需要从 `origin/master` 创建一个 `hotfix/xxx` 分支：
...
在你修复 BUG 之后，你应该把你 `commit` 的代码 `push` 到远程分支，如 `origin/hotfix/fix_doc`：
...
此时你在 `hotfix` 分支的测试环境测试没有问题之后，应在 `gogs` 对应项目的 `origin/master` 和 `origin/develop` 发起合并请求，以范例项目（http://git.epweike.net:3000/suyaqi/teach.epweike.net/pulls）为例：
...
这时需要找到不符合规范的那次提交 xxx，然后 `reset` 到 xxx 的前一个分支：
...
所以我们强烈建议依据 [提问的智慧](https://github.com/ryanhanwu/How-To-Ask-Questions-The-Smart-Way/blob/master/README-zh_CN.md) 推荐的方式来提问。
